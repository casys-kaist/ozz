...
unsigned long get_breakpoint_addr(void) {
  char buf[128];
  FILE *fp = popen(
      "grep 'kssb_test_breakpoint' /proc/kallsyms | head -n 1 | cut -d' ' -f1",
      "r");
...
unsigned long get_breakpoint_addr2(void) {
  char buf[128];
  FILE *fp = popen(
      "grep 'kssb_test_breakpoint2' /proc/kallsyms | head -n 1 | cut -d' ' -f1",
      "r");
  fgets(buf, sizeof(buf), fp);
  pclose(fp);
  return strtoul(buf, NULL, 16);
...
#ifdef _KCOV
  Read number of PCs collected.n = __atomic_load_n(&cover[0], __ATOMIC_RELAXED);
  for (i = 0; i < n; i++)
    printf("0x%lx\n", cover[i + 1]);
