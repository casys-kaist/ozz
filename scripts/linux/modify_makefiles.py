...
def do_modify_makefile(kernel, directive, path, makefile, misc=None):
    ...
    if not os.path.isfile(makefile):
        print("\033[31m" + "[-]" + "\033[0m" + " Makefile does not exist: {}".format(makefile), file=sys.stderr)
        return
    if fil and not os.path.isfile(os.path.join(kernel, path)):
        print("\033[31m" + "[-]" + "\033[0m" + " Basetarget does not exist: {}".format(path), file=sys.stderr)
        return
    with open(makefile, 'r+') as f:
